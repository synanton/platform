...
**GPU plane update (2026-09-24):** the cluster is built and up (workers currently cordoned between projects), and GPU-5/GPU-7 are no longer a blank slate:

- **GPU-5 deployment package exists and is review-revised** (PR #15): **one inference workload per physical GPU** — node1 = TEI embedding (GTX 1650, BGE-base fp16; TEI `turing-1.9` build, sm_7.5) + CPU-only Gateway/Envoy/PostgreSQL, node2 = vLLM reranker (RTX 4060 Ti, Qwen3-Reranker-0.6B), node3 = vLLM synthesis (RTX 5060 Ti, Qwen3-4B-Instruct-2507). Blueprint manifests + Helm chart + scripts under `deployments/homelab/`; the invalid two-container/one-GPU colocation was retracted (device plugins allocate per container — no pod-level GPU inheritance).
- **Model state (verified 2026-09-24):** all Qwen3 weights in place (`/mnt/local-fast/models/...`, bf16); `bge-base-en-v1.5` complete incl. tokenizer files on **all** nodes (operator mirrors models everywhere); `bge-small-en-v1.5` fallback partial. Manual downloads use a `uv` venv (`uv venv --python 3.12 --seed`) with `HF_ENDPOINT=https://hf-mirror.com` as the workaround for huggingface.co SSL EOF failures.
- **GPU-7 routing/runtime layer implemented and unit-tested** (22 tests): provider registry (`gpu-gateway.providers.<id>`), generic OpenAI-compatible provider runtime (mock provider included), authoritative `ProviderRouter` (fail-closed startup, no local fallback in external mode, kill switch), logical→provider model-ID rewriting on every downstream body **including SSE chunks**, SSE relay with exactly-one `[DONE]` + terminal usage, per-provider circuit breaker. Compose datasource env names fixed (`GPU_GATEWAY_DB_*`).
- **Still blocked:** GPU-5 cluster bring-up (phases 0–9, pending); GPU-7 §37 acceptance — the public OpenAI-compatible HTTP face (`:8080`) is not in the build yet (live surface is gRPC :9090), so T02/T03 of the retrieval benchmark stay blocked until either GPU-5 bring-up or the GPU-7 HTTP-face ticket lands. GPU-7's budget/sensitivity/cost-ledger/health-scheduler keys are declared config, not enforced (T-K8S-38/46/48/48b/49).
