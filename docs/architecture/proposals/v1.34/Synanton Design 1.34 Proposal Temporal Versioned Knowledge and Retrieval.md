# Review: Synanton Design 1.34 - Temporal Versioned Knowledge and Retrieval

**Review Date:** 2026-09-12
**Reviewer:** Architecture Review Board
**Status:** Revision Review
**Recommendation:** Accept - implementation-ready after minor clarifications

------

## Executive Summary

Design 1.34 is a **substantial and successful response** to the architecture review. All twelve prior concerns have been addressed with concrete, normative solutions. The design now provides a complete cross‑plane temporal semantics contract that integrates cleanly with Designs 1.23, 1.25, 1.26, 1.27, 1.28, 1.29, 1.30, 1.31, and 1.32.

The revision demonstrates disciplined architecture: ownership is assigned unambiguously, normative invariants are explicit, failure semantics are defined, and performance considerations are addressed without over‑constraining implementation. The design is now ready for implementation planning.

A small number of **minor clarifications** remain, none of which block acceptance.

------

## New Observations and Minor Gaps

The following are **minor clarifications**, not blocking issues.

### 1. Publication Date Is Missing as a Distinct Temporal Dimension

Legal examples in the original design referenced `published` as a distinct concept. The revision retains `observed_at` and validity, but not `published_at`. In legal contexts, three temporal dimensions frequently matter:

text

```
published_at    - when the source declared the version to the world
observed_at     - when Synanton knew about it
valid_from / valid_to - when it was applicable
```
...
- A document may be published but not yet observed by Synanton.
- A document may be published before being legally effective (the amendment example in §28).
- Publication date is often cited in legal reasoning.

**Recommendation:** Add `published_at` as an optional source‑provided metadata field, distinct from `observed_at`. This preserves the legal provenance without adding complexity.

------

### 2. Overlapping Validity Within a Single Series: What Does Search Return?

§9 correctly states that overlapping validity must be retained and flagged, not silently resolved. But it does not define what a search actually returns in that case.
...
A query `as_of = 2025-03-15` finds both v5 and v6 temporally eligible.

Does the search:

- Return both versions as candidates?
- Return only the one with the later `valid_from`?
- Return both and let the applicability layer decide?
- Flag the ambiguity in the result?

**Recommendation:** Add a brief clarification that overlapping within a series is flagged and both versions are returned to the caller with an explicit overlap indicator. The platform does not silently select.

------

### 3. Temporal Metadata Updates vs. Knowledge Recalculation

§14.2 states that a correction triggers impact analysis and recalculation. This is correct for corrections that change the semantic meaning of derived knowledge.

However, a correction that changes only `valid_from` (e.g., from 2025‑02‑01 to 2025‑02‑15) does not change the content of the affected knowledge - it only changes the temporal metadata on the search projection. Recalculation in the Equalix sense is unnecessary; a metadata update on the projection suffices.

The design currently conflates two operations:

text

```
correction → metadata-only update (cheap, no recalculation)
correction → content change (requires recalculation)
```

**Recommendation:** Clarify in §14.2 that corrections that change only temporal metadata on the source version require a projection metadata update, not full derived-knowledge recalculation. Corrections that change content or semantic meaning trigger the full Resolutor/Equalix path.

------

### 4. Lifecycle State Transition Rules Are Implicit

§15 lists lifecycle states (`ACTIVE`, `SUPERSEDED`, `RETENTION_EXPIRED`, `LEGAL_HOLD`, `DELETED`, `CORRECTED`) but does not define legal transitions.

For example:

- Can a version move from `DELETED` to `LEGAL_HOLD`? (Probably not.)
- Can a version move from `RETENTION_EXPIRED` to `ACTIVE`? (Probably only via explicit reinstatement.)
- Is `CORRECTED` a terminal state, or can a corrected version be corrected again?
- Can `SUPERSEDED` coexist with `LEGAL_HOLD`?

**Recommendation:** Add a small state transition table in §15. This is a normative concern because it affects how downstream planes interpret lifecycle state.

------

### 5. Interaction with Design 1.31 Projection Generations

Design 1.31 defines projection generations for atomic index activation. When temporal metadata changes on a version (via correction or lifecycle change), does this require a new projection generation, or an in‑place update?

The design does not address this. The answer likely depends on the backend:

- Small metadata changes may be applied in place.
- Large structural changes (e.g., a correction affecting many versions) may warrant a new generation.

**Recommendation:** Add a brief note in §20 or §35 that temporal metadata updates follow the projection lifecycle rules of Design 1.31, with generation rebuild reserved for structural changes.

------

### 6. Source Re‑identification Across Version Series

If a source is renumbered or reissued under a different identifier (e.g., a regulation consolidated into a new statutory instrument), is the new identifier a continuation of the same `VersionSeries`, a new series, or a linked series?

This is a domain‑specific concern, but the design could note that:

- Version series continuity is a source‑declared or domain‑policy decision.
- The platform does not automatically merge series based on content similarity.

**Recommendation:** Add a brief note in §3 or §42 (open questions) acknowledging this as a domain concern.
...
1. **Explicit ownership assignment** - §2 is now unambiguous: Ingestion owns source history, Knowledge owns derived knowledge, Search owns retrieval behavior. This is exactly the right boundary.
2. **Normative time zone semantics** - §5 is one of the strongest additions. The refusal to invent UTC interpretation for date‑only legal statements is precisely correct for legal auditability.
3. **"Current" as a set** - §8, invariant 10, invariant 11, invariant 24 together eliminate the most common failure mode in temporal systems: assuming one globally current version.
4. **Two‑stage temporal resolution** - §20.3 and §22 provide a clear, scalable pattern: `(source_id, as_of) → version_id → semantic retrieval`. This is the right architecture.
5. **Correction model** - §14 correctly distinguishes corrected interpretation from historical observation. This is essential for legal reproducibility.
6. **Failure semantics** - §32 provides explicit failure states (`VERSION_NOT_FOUND`, `HISTORY_INCOMPLETE`, `VERSION_DELETED_BY_POLICY`, etc.) that prevent silent fallback.
7. **Invariant set** - 25 invariants codify the design's normative requirements. They are consistent, non‑redundant, and well‑scoped.
8. **Open questions are genuinely open** - §42 lists items that are implementation choices, not unresolved semantic gaps. This is the right discipline.

### Consistency with the Broader Design Series

| Design             | Interaction                                                  | Assessment   |
| ------------------ | ------------------------------------------------------------ | ------------ |
| 1.23 Security      | Temporal filtering is independent of and does not weaken authorization | ✅ Consistent |
| 1.25 Knowledge     | Knowledge remains derived from a specific source version; corrections trigger Resolutor/Equalix | ✅ Consistent |
| 1.26 Content Cache | Content retention is distinct from source‑version metadata retention; legal hold propagates across layers | ✅ Consistent |
| 1.27 Eventing      | Version observation events use the common eventing substrate; corrections are source‑state facts | ✅ Consistent |
| 1.28 Ingestion     | Version series ownership is explicitly assigned here         | ✅ Consistent |
| 1.29 Identity      | No identity semantics change                                 | ✅ Consistent |
| 1.30 AI Runtime    | AI provenance remains separate from source temporal provenance | ✅ Consistent |
| 1.31 Search        | Temporal eligibility joins security eligibility as a pre‑ranking constraint | ✅ Consistent |
| 1.32 Platform API  | API semantics are constrained but not frozen; field names deferred | ✅ Consistent |
| 1.33 Kubernetes    | No Kubernetes‑specific behavior introduced                   | ✅ Consistent |
...
Revision 1.1 of Design 1.34 fully addresses all prior review concerns and is architecturally sound. The design:

- Correctly assigns ownership across planes.
- Provides normative time zone semantics.
- Defines `current` as a set, not a single version.
- Specifies correction, deletion, legal hold, and lifecycle behavior.
- Establishes a scalable temporal index strategy.
- Constrains API semantics without freezing implementation.
- Introduces 25 coherent normative invariants.

The remaining observations (publication date, overlap behavior within a series, correction metadata vs. recalculation distinction, lifecycle transitions, projection generation interaction, series re‑identification) are **minor clarifications** that can be resolved during implementation planning or in a subsequent revision. None are blocking.

**Recommendation:** Proceed to implementation planning. The design is ready to be integrated into the Platform Architecture 1.x consolidation document and can be implemented alongside Design 1.32 (Platform API) and the ongoing Kubernetes compatibility review.

The strongest architectural contribution of this design is its clear separation:
...
This is exactly the boundary required for historical RAG, temporal retrieval, legal change analysis, and reproducible point‑in‑time answers at platform scale.

create Design 1.34 v1.2 md document with last minor fixes

The user wants me to create Design 1.34 v1.2 as a markdown document with the last minor fixes incorporated. The minor fixes from my review were:

1. Add `published_at` as an optional source-provided metadata field, distinct from `observed_at`
2. Clarify overlapping within a series returns both versions with an explicit overlap indicator
3. Clarify in §14.2 that corrections that change only temporal metadata require a projection metadata update, not full derived-knowledge recalculation
4. Add a small state transition table in §15 for lifecycle states
5. Add a brief note that temporal metadata updates follow the projection lifecycle rules of Design 1.31
6. Add a brief note about source re-identification across version series

I need to produce the full v1.2 document incorporating these fixes while preserving the rest of the v1.1 content. Let me write it out carefully, keeping the structure and content intact but adding these clarifications.

Let me produce a complete, well-structured document.

# Synanton Design 1.34 - Temporal Versioned Knowledge and Retrieval

> **Document type:** Architecture proposal
> **Design:** 1.34
> **Revision:** 1.2 - incorporates final review clarifications
> **Status:** Proposal - implementation-ready
> **Date:** 2026-09-12
> **Primary domains:** Legal, regulatory, contracts, policies, standards, controlled documents
> **Related designs:** 1.23 Security, 1.25 Knowledge, 1.26 Content Cache, 1.27 Eventing/Workflow, 1.28 Ingestion, 1.29 Identity, 1.30 AI Runtime, 1.31 Search, 1.32 Platform API
...
- Synanton Platform Architecture 1.0:
  https://github.com/synanton/platform/blob/main/docs/architecture/synanton-platform-architecture-1.0.md
- Design 1.23 - Security
- Design 1.25 - Knowledge
- Design 1.26 - Content Cache
- Design 1.27 - Eventing and Workflow
- Design 1.28 - Ingestion
- Design 1.29 - Identity
- Design 1.30 - AI Runtime
- Design 1.31 - Search
- Design 1.32 - Platform API